...
public class ConvertArrayReducedForm {
    ...
    static void convert(int[] arr, int n) {
        ...
        /*
        In the code provided, the line "int[] duplicateArray = arr;" creates a reference to the same array in memory as the original array "arr". It does not create a new copy of the array. Therefore, when the FirstPhaseLearning.Arrays.sort() method is called on "duplicateArray", it sorts the same array in memory that "arr" references.
...
If you want to sort the duplicateArray separately, you should use the FirstPhaseLearning.Arrays.copyOf() function to create a new copy of the array.
For example, int[] duplicateArray = FirstPhaseLearning.Arrays.copyOf(arr, arr.length);
...
This will create a new copy of the original array, so sorting the duplicateArray will not affect the original array.
        ...
        //FirstPhaseLearning.Sorting the array
